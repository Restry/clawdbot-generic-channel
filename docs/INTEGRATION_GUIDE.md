...
面向第三方接入方：H5 页面、聊天 App、uni-app、Taro、微信小程序如何接入 `generic-channel`，把前端会话接到 OpenClaw。
...
注意真实频道 ID 是 `generic-channel`，不是 `generic`。
...
轻量级状态接口，只返回 `generic-channel` 自身运行状态：
...
`generic-channel` 是一个很薄的聊天协议层，任何能发 WebSocket JSON 的客户端都能接：H5、iOS/Android、uni-app、Taro、React Native、Flutter、微信小程序。
...
| 问题 | 说明 |
|------|------|
| 频道名写成 `generic` | 正确配置键是 `channels.generic-channel` |
